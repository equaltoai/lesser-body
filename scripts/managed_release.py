...
def base_script_inputs(version: str) -> list[dict[str, Any]]:
    return [
        ...
        {"name": "soul_binding_integration_bearer_secret_arn", "required": True, "description": "Required exact Secrets Manager ARN for the dedicated Body/Ptah/Ba to Lesser soul-binding bearer. Without it, agent_local_install_plan and agent_bind_soul both fail closed with not_configured. Lesser-body injects LESSER_SOUL_BINDING_INTEGRATION_BEARER_ARN on the instance MCP Lambda and grants direct read access to that secret."},
...
def base_template_parameters(auxiliary_assets: list[dict[str, Any]]) -> list[dict[str, Any]]:
    params: list[dict[str, Any]] = [
        ...
        {"name": "LesserSoulBindingIntegrationBearerSecretARN", "required": True, "default": "", "description": "Required managed-deploy prerequisite: exact Secrets Manager ARN for the dedicated Body/Ptah/Ba to Lesser soul-binding bearer. Without it, agent_local_install_plan and agent_bind_soul both fail closed with not_configured. The empty template default remains only for release-template compatibility; the release helper forwards --soul-binding-integration-bearer-secret-arn (or $LESSER_SOUL_BINDING_INTEGRATION_BEARER_ARN) into this parameter."},
