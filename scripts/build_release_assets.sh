...
' "$(go list -f '{{.Dir}}' github.com/theory-cloud/apptheory/v3/runtime/mcp)/server.go")"
if [[ -z "${MCP_PROTOCOL_VERSION}" ]]; then
  echo "failed to resolve MCP protocol version from github.com/theory-cloud/apptheory/v3/runtime/mcp" >&2
  exit 1
fi
