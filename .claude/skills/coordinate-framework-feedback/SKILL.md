...
- **AppTheory v0.25.0** — especially the **MCP server runtime** (how Lambda-hosted MCP servers are structured, how tools / resources / prompts register, how `.well-known/mcp.json` is generated, how OAuth protected-resource metadata is served, how JSON-RPC dispatch works). Steward: Theory Cloud AppTheory steward.
- **TableTheory v1.6.0** — DynamoDB ORM, single-table tag semantics, used for per-actor MCP session persistence and memory tools. Steward: Theory Cloud TableTheory steward.
