...
description: "Use when Codex handles GitHub activity for equaltoai/lesser-body and should preserve routed-agent provenance: reading issues or PRs, commenting, reviewing, creating agent-scoped branches, committing bounded file changes, opening PRs, or creating truthful check runs. Prefer the mcp__body_lab__ GitHub tools whenever they can perform the needed operation; fall back to the general GitHub plugin or gh only for capabilities the routed tools do not expose."
---
...
Use the routed `mcp__body_lab__` GitHub tools first so GitHub activity is attributable to this body steward endpoint. Preserve body's MCP-contract-first, scope/profile-rigorous, lesser-integration-respectful, host-delegation-disciplined, and AGPL discipline; provenance improves traceability but does not relax any gate.

## Tool preference

1. Prefer `mcp__body_lab__` for supported GitHub actions:
   - `github_list_repos`
   - `github_get_issue`
   - `github_get_pr`
...
- Preserve body discipline: no bypassing required review, no force pushes, no production deploy actions, no live deploys without successful lab/staging soak, no scope/profile bypasses, no dynamic tool registration, and no MCP contract breakage without explicit coordination.
...
```markdown
---
Provenance: posted via body steward routed GitHub tool (`mcp__body_lab__`).
```

### Branches, commits, and PRs

- Create GitHub branches with `github_create_branch` when publishing work through the routed endpoint. Use a short bounded `branch_suffix` such as `fix-mcp-discovery` or `docs-github-provenance`.
...
- Open PRs with `github_open_pr` from the routed branch when possible. PR bodies must include body-relevant contract notes: MCP contract impact, scope/profile impact, lesser integration and SSM-export impact, host-delegation and PII/logging impact, AGPL/dependency impact, validation run, and rollout/soak notes where applicable.
- Do not mark a PR ready for review unless the repo-required local gates have passed when applicable. The common body gates are:

```bash
go test ./...
bash scripts/build.sh
CDK_DEFAULT_ACCOUNT=000000000000 CDK_DEFAULT_REGION=us-east-1 \
  bash -lc 'cd cdk && npm ci && npx cdk synth -c app=lesser -c stage=dev -c baseDomain=example.com'
bash scripts/check_cdk_discovery_routes.sh
```

Release-asset or managed-deploy changes may also need the release regression scripts used by CI.

### Check runs

- Use `github_create_check_run` only for truthful, completed steward evidence on a specific SHA.
- Use a distinct name such as `body-steward/local-validation`; never impersonate GitHub Actions or CI jobs.
- Set `success` only when the named commands actually passed. Use `failure`, `neutral`, or `skipped` when that is the truthful result.
- Summaries should list exact commands and notable omissions. Do not claim remote deployment, live soak, MCP canary evidence, or CI success from local checks.
...
- Validation actually run, with explicit note if the full body gate was not run.
