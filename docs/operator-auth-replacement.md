...
- `LESSER_HOST_INSTANCE_KEY` remains a server-to-server credential for lesser-body calls into lesser-host, including host-backed communication and scoped x402 grant validation
...
4. retain `LESSER_HOST_INSTANCE_KEY` only inside the lesser-body deployment for host-backed communication calls and scoped x402 grant validation
