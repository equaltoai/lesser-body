...
func (c *Client) ListArticles(ctx context.Context, bearerToken string, first int, after string, authorID string, includeContent bool) (*ArticleConnection, error) {
	...
	resp, err := c.Execute(ctx, bearerToken, Operation{
		Query:         "query BodyArticles($authorId: ID, $first: Int, $after: Cursor) { articles(authorId: $authorId, first: $first, after: $after) { edges { cursor node { " + articleFields(includeContent) + " } } pageInfo { hasNextPage hasPreviousPage startCursor endCursor } totalCount } }",
