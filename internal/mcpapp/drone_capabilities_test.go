...
func TestDroneRuntimeCapabilityContractIsExplicit(t *testing.T) {
	...
	t.Run("tools list exposes drone-only runtime surface", func(t *testing.T) {
		...
		if !have["post_create"] || !have["post_get"] || !have["conversation_get"] || !have["notification_get"] || !have["memory_query"] || !have["soul_read"] || !have["skills_catalog"] || !have["skill_bundle_get"] {
