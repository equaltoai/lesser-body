...
func New(name, version string) (*apptheory.App, error) {
	...
	actorHandler := WithBrowserCORS(WithClientCompatibilityHeaders(WithMCPAuthorization(WithActorBinding(WithRuntimePolicy(WithAudit(WithToolContext(WithSessionListenerTimeoutBudget(srv.Handler())), logger))))))
